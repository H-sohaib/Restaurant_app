    ...
    <section class="relative flex flex-nowrap ml-auto mr-auto mt-0 lg:items-center w-11/12">
        <div class="w-full py-5 md:py-2 lg:w-1/2 mr-3 overflow-y-auto">
            ...
            <div class="max-w-lg text-center">
                <h1 class="text-1xl font-bold sm:text-2xl">Edit Plate {{ $menu->name }} !</h1>

                <p class="mt-1 text-gray-500">
                </p>
            </div>

            <form method="POST" action="{{ route('admin.menus.update', ['menu' => $menu->id]) }}"
                ...
                <div>
                    ...
                    <x-admin.text-input id="name" name="name" type="text" class="mt-1 block w-full" required
                        autofocus autocomplete="name" :value="old('menu', $menu->name)" />
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        ...
                        placeholder="Write your description here..."> {{ $menu->description }}</textarea>
                ...
                <div>
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input name="special" type="checkbox" @checked($menu->special) value="1"
                            class="sr-only peer">
                        <div
                            class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300  rounded-full peer  peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all  peer-checked:bg-blue-600">
                        </div>
                        <span class="ml-3 text-sm font-medium text-gray-900 ">Special menu</span>
                    </label>
                </div>
                {{-- plate price --}}
                <div>
                    <x-admin.input-label for="name" label="Price" />
                    <x-admin.text-input name="price" type="text" class="mt-1 block w-full" required autofocus
                        autocomplete="number" placeholder="write the price by DH" :value="old('menu', $menu->price)" />
                ...
                <div>
                    ...
                    <select id="category" name="category_id" required
                        ...
                        @foreach ($categories as $category)
                            <option @selected($menu->category_id == $category->id) value="{{ $category->id }}">{{ $category->name }}
                            </option>
                        @endforeach
                    </select>
